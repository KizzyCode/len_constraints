...
#[test]
fn test_relative() {
	Relative::<u8, Sub, _4>::from(s!(4)).get_slice(8).unwrap();
	Relative::<u8, Sub, _8>::from(s!(9)).get_slice(17).unwrap();
}
#[test]
fn test_relative_err() {
	Relative::<u8, Sub, _4>::from(s!(0)).get_slice(3).unwrap_err();
	Relative::<u8, Sub, _4>::from(s!(3)).get_slice(8).unwrap_err();
	Relative::<u8, Sub, _8>::from(s!(9)).get_slice(16).unwrap_err();
}